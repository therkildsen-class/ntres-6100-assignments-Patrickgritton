...
group_by(winner) |>
summarise(total_votes = sum(value))

current_votes |> 
ggplot(mapping = aes(x = winner, y = total_votes, fill = winner)) +
geom_col() +
labs(
...
total_pop <- q_2b |> 
  ...
  total_electoral_votes <- q_2b |> 
  filter(Metric == "electoral_votes") |> 
  summarise(total_evs = sum(value, na.rm = TRUE)) |> 
  pull(total_evs)
  
  proportional_votes <- q_2b |> 
  filter(Metric == "population") |> 
  mutate(proportional_evs = value / total_pop * total_electoral_votes) |> 
  group_by(winner) |> 
  summarise(total_proportional_evs = sum(proportional_evs))
  
  ggplot(proportional_votes, aes(x = winner, y = total_proportional_evs, fill = winner)) +
  geom_col() +
  labs(
    title = "Electoral Votes Proportional to Population",
    x = "Candidate",
    y = "Total Electoral Votes (Proportional)"
  ) +
  theme_minimal() +
  theme(legend.position = "none")
```

![](b1bf5525-7d37-427a-b56b-da368061098d.png)

![](9e29a26b-118d-4612-b849-d510eb16a0c2.png)
...
other = sum(othersVotes + johnsonVotes + steinVotes + mcmullinVotes, na.rm = TRUE))
...
``` r
q_3b |> 
filter(Metric == "popular_vote") |> 
ggplot(mapping = aes(x = winner, y=value, fill = winner)) +
geom_col()
```

![](c56a1184-e7bc-45b6-abcb-45dc3239dbd1.png)
...
![](f74368cd-f19c-4fa1-a3de-9ff82f062eae.png)
